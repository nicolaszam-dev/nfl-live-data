export default async function handler(req, res) {
  try {
    ...
    // Team → conference/division is permanent NFL structure, safe to hardcode.
    const DIVISIONS = {
      ...
      HOU:['AFC','AFC South'], IND:['AFC','AFC South'], JAX:['AFC','AFC South'], TEN:['AFC','AFC South'],
      DEN:['AFC','AFC West'], KC:['AFC','AFC West'], LV:['AFC','AFC West'], LAC:['AFC','AFC West'],
      DAL:['NFC','NFC East'], NYG:['NFC','NFC East'], PHI:['NFC','NFC East'], WSH:['NFC','NFC East'],
      ...
      ARI:['NFC','NFC West'], LAR:['NFC','NFC West'], SF:['NFC','NFC West'], SEA:['NFC','NFC West']
    };
    ...
    function walk(node) {
      if (node.standings && Array.isArray(node.standings.entries)) {
        node.standings.entries.forEach(entry => {
          ...
          if (!seen[abbr]) seen[abbr] = entry; // first occurrence is fine
        });
      }
